...
lazy_static! {
    pub static ref DEFAULT_IMPORT_COMMENT: String = format!("imported on {}",format_datetime(&Local::now().naive_utc()));
}
...
macro_rules! opt{ ($e:expr) => ($e.ok_or_else(||Error::NoValue(""))? ) }
macro_rules! get_string{ ($e:expr,$b:expr) => (opt!($e.get($b)).to_string()) }
...
impl<'a> ImportParser<'a> {
    ...
    pub fn new(path: &Path, default_date_name: NaiveDateTime, date_parse_format: &'a str) -> Result<ImportParser<'a>,Error> {
...
fn parse_date(input: &str, format: &str) -> Result<NaiveDate,Error> {    
    match NaiveDate::parse_from_str(input, format) {
        Ok(v) => Ok(v),
        Err(e) => {error!("Unable to parse date in {} with format {}",input,format);
            Err(e.into())
        }
    }
}
...
fn parse_record(parser: &ImportParser, record: &StringRecord, header: &StringRecord) -> Result<ImportMembership,Error> {
    ...
    fn next(&mut self) -> Option<Result<ImportMembership,Error>> {
        match self.reader.next() {
            Some(Ok(record)) => {
                ...
                match result {
                    Ok(_) => (),
                    Err(ref e) => {info!("{} on row {:?}",e,record); }
                }
                Some(result)
            },
...
fn bool_from_string(input: &str) -> Result<bool, Error>{
    match input {
        "VIP" | "true" => Ok(true),
        "nVIP" | "false" => Ok(false),
        other => Err(Error::Parser(format!("Unexpected value: {} wanted {}",other,"VIP or nVIP"))),
    }
}
...
mod test {
    ...
    fn parse_account_header_correct_long_unending() {
        let path = get_path_for_existing_file("src/test_csv_header_valid_long_unending.csv").unwrap();
        let mut var = Vec::new();
        ...
        var.push(ImportMembership {
            ...
            memberships: vec![Membership{
                ...
                },Membership{ // second date, doesn't close in parse_record loop, end of data
